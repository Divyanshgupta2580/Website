...
export default function Badge({
  ...
  const variantStyles = {
    ...
    surface: "bg-white text-[#20272D] border border-[#D5D4D0] font-medium",
    outline: "bg-transparent text-[#66717A] border border-[#D5D4D0] font-medium",
  };

  return (
    <span
      className={`inline-flex items-center px-2.5 py-1 text-[11px] uppercase tracking-wider rounded-none ${variantStyles[variant]} ${className}`}
    >
      {children}
    </span>
  );
